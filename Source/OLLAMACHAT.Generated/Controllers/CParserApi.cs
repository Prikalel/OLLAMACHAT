...
/// </summary>
[ApiController]
public class CParserApiController(IMediator mediator, IMapperInterface mapper) : ControllerBase
{
    /// <summary>
    ...
    public async Task<IActionResult> GetInitFiles()
    {
        ...
        var res = await mediator.Send(new GetInitFiles.Query());
    ...
    {
        (List<string> result, ErrorResponse? error) = await mediator.Send(new ResolveImportPathDto.Query(mapper.Map(body)));
        if (error != null)
        {
            return StatusCode(400, mapper.Map(error!));
        }
